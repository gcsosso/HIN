...
subroutine read_input(eflag,sfile,tfile,fframe,stride,lframe,outxtc,hw_ex,switch_zdens,ns,ws,n_ws,zmin,zmax,dz, &
                      ...
                      switch_nh,nh_bins,nh_rcut,switch_t_order)

implicit none

integer :: stride, lframe, eflag, wcol, ohstride, pmpi, nxy
integer :: ns, r_ns, fframe, i, npairs, npairs_cn, b_bins, maxr, maxr_RINGS, nr, min_npts, gr_ws, gr_bins, gr_min_dx, nh_bins
...
real :: o_zmin, o_zmax, o_dz, hbdist, hbangle, c_rcut, min_delta, gr_min_dy, nh_rcut
...
character*3 :: switch_water, switch_hbck, switch_cryo, switch_hydration, switch_gr, switch_nh, switch_t_order
...
read(100,*) buffer, nh_rcut           ; if (trim(adjustl(buffer)).ne.'RMAX') eflag=1
read(100,*) buffer, switch_t_order      ; if (trim(adjustl(buffer)).ne.'Q') eflag=1
