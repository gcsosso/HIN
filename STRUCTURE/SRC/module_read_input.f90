...
subroutine read_input(ARG_LEN, sfile, tfile, fframe, lframe, stride, switch_outxtc, switch_progress, ns, ws, &
   ...
   ! RADIAL
   logical(1) :: switch_rad, switch_rad_cn, switch_rad_smooth, switch_rad_pdf, switch_rad_nh
   ...
   ! HISTOGRAM
   logical(1) :: switch_hist, switch_hist_noh
   character(5) :: hist_x
   ...
   do i=1,num_categories
      ...
      if (args(i,1).eq.'trajectory') then
         ...
         switch_rad = .true.
         do j=2,num_args(i) ; if (args(i,j).eq.'') exit
            call read_radial_arg(args(i,j), eflag, .true._1, rad_ws, rad_bins, rad_min, rad_max, rad_nh_cut, switch_rad_cn, switch_rad_smooth, switch_rad_pdf, switch_rad_nh)
         end do
         ...
         switch_hist = .true.
         do j=2,num_args(i) ; if (args(i,j).eq.'') exit
            call read_hist_arg(args(i,j),eflag, .true._1 ,hist_x,hist_centre,hist_min,hist_max,hist_nbins,switch_hist_noh)
         end do
