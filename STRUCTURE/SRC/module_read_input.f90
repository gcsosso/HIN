...
subroutine read_input(ARG_LEN, sfile, tfile, fframe, lframe, stride, switch_outxtc, switch_progress, ns, ws, &
   ...
   do i=1,num_categories
      ...
      if (args(i,1).eq.'trajectory') then
         ...
         switch_rad = .true.
         do j=2,num_args(i) ; if (args(i,j).eq.'') exit
            call read_radial_arg(args(i,j), eflag, .true._1, rad_ws, rad_bins, rad_min, rad_max, switch_rad_cn, switch_rad_smooth)
         end do
         ...
         do j=2,num_args(i) ; if (args(i,j).eq.'') exit
            switch_lsi = .true.
            call read_lsi_arg(args(i,j), eflag, .true._1, lsi_bins, lsi_rcut)
         end do
