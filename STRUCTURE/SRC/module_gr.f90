...
subroutine gr(pos,list_ws,o_ns,cart,icell,list_nw,n_nw,n_ow,gr_ws,gr_bins,dr,half_dr,rad,gr_mol_norm,gr_atm_norm,fact,o_dist)
...
integer :: o_ns, cart, n_nw, n_ow, gr_ws, gr_bins
...
if (gr_ws.eq.0) then ! for O-O PCF [0]
  do i=1,n_ow
...
elseif (gr_ws.eq.1) then ! For M-O PCF: individual atoms [1]
  do i=1,n_nw
    i_spc=list_nw(i)
    i_pos(1)=pos(1,i_spc) ; i_pos(2)=pos(2,i_spc) ; i_pos(3)=pos(3,i_spc)
...
elseif (gr_ws.eq.2) then ! For M-O PCF: whole molecule, then averaged over atoms (to account for multiple counting) [2]
  do i=1,n_nw
    i_spc=list_nw(i)
    i_pos(1)=pos(1,i_spc) ; i_pos(2)=pos(2,i_spc) ; i_pos(3)=pos(3,i_spc)
...
elseif (gr_ws.eq.3) then ! For M-O PCF: whole molecule, treated as single particle (using smallest M-Ow distances) [3]
  do i=1,n_nw
    i_spc=list_nw(i)
    i_pos(1)=pos(1,i_spc) ; i_pos(2)=pos(2,i_spc) ; i_pos(3)=pos(3,i_spc)
...
if (gr_ws.eq.0) then ! For O-O PCF [0]
  num_i=dble(n_ow)
elseif (gr_ws.eq.2) then ! For M-O PCF (averaged over atoms [2])
  num_i=dble(n_nw)
elseif ((gr_ws.eq.1).or.(gr_ws.eq.3)) then ! For M-O PCF (individual atoms [1] or as single particle [3])
  num_i=1.0d0
endif

if ((gr_ws.eq.0).or.(gr_ws.eq.2).or.(gr_ws.eq.3)) then ! For O-O PCF [0] or M-O PCF (averaged [2] or minimum distance [3])
  do ir=1,gr_bins
    r2=(rad(ir))**2.0d0
    gr_tmp=gr_mol(ir)/(fact*r2*num_i)
    gr_mol_norm(ir)=gr_mol_norm(ir)+gr_tmp
  enddo
elseif (gr_ws.eq.1) then ! Use for M-O PCF (individual atoms [1])
  do i=1,n_nw
    do ir=1,gr_bins
      r2=(rad(ir))**2.0d0
