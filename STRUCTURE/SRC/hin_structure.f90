...
integer :: nsix, r_flag, r_flag2, r_flag3, npairs, npairs_cn, flag, patch, o_nz, n_nw
...
call read_input(eflag,sfile,tfile,fframe,stride,lframe,outxtc,hw_ex,switch_zdens,ns,ws,n_ws,zmin,zmax,dz, &
                ...
                o_zmax,o_dz,switch_water,switch_hbck,hbdist,hbangle,thrSS,switch_cryo,c_rcut)
...
if (trim(adjustl(switch_cryo)).eq.'yes') then
   call cryo_alloc(pos,nat,sym,ns,n_ws,list_ws,o_dist,o_ns,list_nw,n_nw)
endif
...
do while ( STAT==0 )
   if (mod(counter,stride).eq.0.and.counter.ge.fframe.and.counter.le.lframe) then
      ...
      ! Cryo...
      if (trim(adjustl(switch_cryo)).eq.'yes') then
         call cryo(pos,sym,ns,n_ws,list_ws,o_dist,o_ns,cart,icell,list_nw,n_nw,c_rcut)
      endif
