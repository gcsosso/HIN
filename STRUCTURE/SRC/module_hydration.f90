...
subroutine hydration_alloc(nat,ns,sym,n_ws,list_ws,o_ns,list_nw,n_nw,n_ow,o_dist,nh_bins,nh_rmax,nh_r,nh_mol,nh_atm)

implicit none

! Arguments
integer :: nat, ns, o_ns, n_nw, n_ow, nh_bins
integer, allocatable :: n_ws(:), list_ws(:,:), list_nw(:), nh_mol(:), nh_atm(:,:)
real :: nh_rmax
...
allocate(nh_r(nh_bins),nh_mol(nh_bins),nh_atm(n_nw,nh_bins))
nh_mol(:)=0
nh_atm(:,:)=0
